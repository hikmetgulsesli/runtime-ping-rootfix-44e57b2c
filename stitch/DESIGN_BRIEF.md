...
- Runtime Ping Rootfix turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page status tile app called Runtime Ping Rootfix. It should show three status cards, a live timestamp, and one reset button. Keep it simple for pipeline testing. REPO: /Users/setrox/projects/runtime-ping-rootfix-20260629-222817. Frontend port: 3621
...
- Record Operations - Runtime Ping Rootfix
- Record Editor - Runtime Ping Rootfix
- Pipeline Board - Runtime Ping Rootfix
- Insights - Runtime Ping Rootfix

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Record Operations - Runtime Ping Rootfix
...
SCREEN_SPEC_2:
- exact_screen_title: Record Editor - Runtime Ping Rootfix
...
SCREEN_SPEC_3:
- exact_screen_title: Pipeline Board - Runtime Ping Rootfix
- surface_id: SURF_PIPELINE_BOARD
- unique_canvas_caption: Pipeline Board: stage lanes, status markers, blockers, aging indicators, ownership, and next-action controls.
- purpose: Help users organize Record work by pipeline stage and board state when those signals are part of the requested product.
- required_content: stage lanes, status markers, blockers, aging indicators, ownership, and next-action controls.
- data_entities: Record, ActivityEvent, Preference
...
- entry_exit_rules: SURF_RECORD_OPERATIONS -> Status changes keep the user in the same Record context and expose recoverable failure feedback.
- design_guidance: Represent board state clearly; do not introduce service-desk queues or unrelated analytics modules.

SCREEN_SPEC_4:
- exact_screen_title: Insights - Runtime Ping Rootfix
...
3. SURF_PIPELINE_BOARD - Pipeline Board
   Purpose: Help users organize Record work by pipeline stage and board state when those signals are part of the requested product.
   Data: Record, ActivityEvent, Preference
   Core content: stage lanes, status markers, blockers, aging indicators, ownership, and next-action controls.
   Actions: ACT_SEARCH_RECORDS (search_input_persistent), ACT_SELECT_RECORD (inline_edit), ACT_UPDATE_RECORD_STATUS (inline_edit)
   Entry/exit: SURF_RECORD_OPERATIONS -> Status changes keep the user in the same Record context and expose recoverable failure feedback.
   Guidance: Represent board state clearly; do not introduce service-desk queues or unrelated analytics modules.
